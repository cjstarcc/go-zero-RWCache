...
const (
	...
	ClusterType = "cluster"
	// MasterType means redis master-slave
	MasterType = "master"
...
type (
	...
	Redis struct {
		SecretKey string // 加密秘钥
		RWMode    string // 读写分离，读模式还是写模式
		DB        int
		Addr      string
		Type      string // 部署模式，node - 单机模式，Master - 主从模式，cluster - 集群模式
		Pass      string
		tls       bool
		brk       breaker.Breaker
		hooks     []red.Hook
...
func NewRedis(conf RedisConf, opts ...Option) (*Redis, error) {
	...
	if conf.Type == MasterType {
		opts = append([]Option{Master()}, opts...)
		if len(conf.RWMode) > 0 {
			opts = append([]Option{WithRWMode(conf.RWMode)}, opts...)
		}
	}
	...
	if !conf.NonBlock {
		if err := rds.checkConnection(conf.PingTimeout); err != nil {
			fmt.Println(fmt.Sprintf("redis connect error, addr: %s", conf.Host))
...
// WithRWMode customizes the given Redis with given RWMode,only for RWMode.
func WithRWMode(RWMode string) Option {
	return func(r *Redis) {
		r.RWMode = RWMode
	}
}
