...
func New(c ClusterConf, barrier syncx.SingleFlight, st *Stat, errNotFound error,
	...
	if c[0].RedisConf.Type == redis.MasterType { // 主从模式中，第一个配置必须是master,如果是主从模式
		cacheRw := CacheRW{}
		cacheRw.dispatcher = hash.NewConsistentHash()
		for _, node := range c {
			if node.RedisConf.RWMode == "slave" { // 如果为主从模式，并且为从节点，那么创建一个读的redis
				cn := NewNode(redis.MustNewRedis(node.RedisConf), barrier, st, errNotFound, opts...)
				cacheRw.dispatcher.AddWithWeight(cn, node.Weight)
			} else if node.RedisConf.RWMode == "master" { // 如果为主从模式,并且为主节点，那么创建一个写的redis
				cacheRw.writeClient = NewNode(redis.MustNewRedis(node.RedisConf), barrier, st, errNotFound, opts...)
			} else {
				fmt.Println("RWRedis config is error!")
			}
		}
		cacheRw.errNotFound = errNotFound
		return cacheRw
	}
